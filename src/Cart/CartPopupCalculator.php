...
/**
 * Server-side cart popup calculation against common cart schemes (Woo cart_popup parity).
 */
final class CartPopupCalculator
{
    /** @var Calculator */
    private $calculator;
    ...
    public function __construct(Calculator $calculator, CartSchemeResolver $resolver, ?CurrencyGate $currencyGate = null)
    {
    ...
    public function calculate(
        ...
        $scheme = $this->findScheme($this->commonSchemes($shop, $cart, $popupType), $kopCode, $months, $filterId);
        if ($scheme === null || !hash_equals(ProductPopupSchemeList::key($scheme), $schemeKey)) {
            throw new UnavailableSchemeException('The selected financing scheme is unavailable.');
        }

        return $this->present($shop, $this->calculator->calculateScheme($shop, $cart->total, $scheme, $firstInstallment));
    }

    /** @param array<string, mixed> $shop @return AvailableScheme[] */
    public function commonSchemes(array $shop, CartContext $cart, string $popupType): array
    {
