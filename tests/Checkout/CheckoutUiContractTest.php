...
assertCheckoutUi(strpos($template, "s='Цена на поръчката'") !== false, 'checkout must use order total label');
assertCheckoutUi(strpos($template, 'страницата на UniCredit') !== false, 'checkout intro must describe UniCredit redirect');
assertCheckoutUi(strpos($template, "s='Брой месеци за погасяване'") !== false, 'months label must be present');
assertCheckoutUi(strpos($template, "s='Първоначална вноска /евро/'") !== false, 'first installment label must be present');
assertCheckoutUi(strpos($template, "s='Обща сума на заема'") !== false, 'loan amount label must be present');
...
assertCheckoutUi(
    (bool) preg_match('/\.unipayment-checkout__select option\s*\{[^}]*color:\s*var\(--unipayment-checkout-red\)/s', $css),
    'checkout select options must use UniCredit red like product/cart popup'
);
assertCheckoutUi(strpos($css, '.unipayment-checkout__select:disabled') !== false, 'checkout select disabled state must keep UniCredit red styling');
assertCheckoutUi(strpos($css, 'font-size: inherit') !== false, 'nested checkout labels must inherit row label size');
assertCheckoutUi(strpos($js, 'previousFirstLocked') !== false, 'checkout scheme change must track locked→editable first-installment reset');
