...
/**
 * UniPayment CLI test runner with explicit suite tiers.
 *
 * Usage:
 *   php tests/run.php safe
 *   php tests/run.php runtime
 *   php tests/run.php destructive
...
$suite = $argv[1] ?? TestSuiteGuard::SUITE_SAFE;
$validSuites = [
    TestSuiteGuard::SUITE_SAFE,
    TestSuiteGuard::SUITE_RUNTIME,
    TestSuiteGuard::SUITE_DESTRUCTIVE,
    TestSuiteGuard::SUITE_ALL,
];
...
putenv(TestSuiteGuard::ENV_SUITE . '=' . $suite);

/** @var list<string> $files */
$files = [];
/** @var list<string> $excludeFromSafe */
$excludeFromSafe = [
    __DIR__ . '/Infrastructure/DevStatePreservationTest.php',
];
...
foreach ($iterator as $fileInfo) {
    ...
    if ($suite === TestSuiteGuard::SUITE_SAFE && in_array($path, $excludeFromSafe, true)) {
        continue;
    }
