...
assertAud009(strpos($coordinator, 'finalizeDefinitiveRemoteFailure') !== false, 'SmartUCF definitive failure handling was removed');
assertAud009(strpos($coordinator, 'markFailed($idOrder)') === false, 'SmartUCF failure still changes native order state');
assertAud009(strpos($coordinator, 'updateOrderStatus(') === false, 'best-effort SmartUCF CP PATCH removed');
...
assertAud009(strpos((string) file_get_contents($root . '/unipayment.php'), "version = '2.0.3'") !== false, 'version 2.0.3');
assertAud009(is_file($root . '/upgrade/upgrade-2.0.3.php'), '2.0.3 upgrade script must exist');
