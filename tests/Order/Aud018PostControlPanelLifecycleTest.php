...
// Test K — controllers use shared service
...
foreach ([$productPopup, $cartPopup, $checkout] as $source) {
    assertAud018(strpos($source, 'PostControlPanelLifecycleService') !== false, 'K: controller uses lifecycle service');
    assertAud018(strpos($source, 'applySmartUcfResultToResponse') === false, 'K: duplicated SmartUCF mapper removed');
}
assertAud018(strpos($productPopup, 'runPostControlPanelLifecycle') !== false, 'K: product popup replay uses shared path');
