...
$cp = new CpFailUxCp();
$cp->queue[] = new HttpException(404, []);
...
try {
    ...
    assertCpFailUx(false, 'C: definitive CP 404 must throw');
...
assertCpFailUx(
    $bankU->updates !== [] && $bankU->updates[0]['statusId'] === BankStatus::SEND_FAILED_CP,
    'D: existing bank-status semantics for ambiguous retained (not changed)'
);
