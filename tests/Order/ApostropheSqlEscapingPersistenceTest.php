...
// ---------------------------------------------------------------------------
// A. Financing snapshot apostrophe round-trip
// ---------------------------------------------------------------------------
...
assertApos(
    is_array($db->lastInsertData[0]['cp_status_sync_status_id'] ?? null)
    && (($db->lastInsertData[0]['cp_status_sync_status_id']['type'] ?? '') === 'sql'),
    'intentional SQL NULL wrappers preserved on insert'
);

// E. Safe name still works
...
// ---------------------------------------------------------------------------
// B. Snapshot update with apostrophe + SQL NULL
// ---------------------------------------------------------------------------
$snapshots->update(42, [
...
assertApos(
    array_key_exists('cp_status_sync_error_class', $db->snapshotRows[42])
    && $db->snapshotRows[42]['cp_status_sync_error_class'] === null,
    'snapshot update preserves intentional SQL NULL'
);
$lastSnapUpdate = $db->lastUpdateData[count($db->lastUpdateData) - 1];
assertApos(
    strpos((string) ($lastSnapUpdate['cp_status_sync_status'] ?? ''), "Bank\\'s status") !== false,
    'snapshot update SQL-escapes free-text status'
);
assertApos(
    is_array($lastSnapUpdate['cp_status_sync_error_class'] ?? null)
    && (($lastSnapUpdate['cp_status_sync_error_class']['value'] ?? '') === 'NULL'),
    'snapshot update keeps raw SQL NULL wrapper'
);

// ---------------------------------------------------------------------------
// C–D. cp_payload apostrophe + retry idempotency
// ---------------------------------------------------------------------------
...
assertApos(
    strpos((string) ($db->lastUpdateData[count($db->lastUpdateData) - 1]['cp_payload'] ?? ''), "Product\\'s Name") !== false,
    'cp_payload SQL-escaped at update boundary'
);
