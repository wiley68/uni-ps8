...
$calculator=new Calculator('2026-08-17'); $shop=calculatorFixture(['uni_eur'=>0]); $cart=new CartContext([new CartLine(new ProductContext(42,[7],1000),3,2,1000)],1000);
$scheme=(new CartSchemeResolver($calculator))->resolve($shop,$cart)->standardSchemes[0]; $calculation=$calculator->calculateScheme($shop,1000,$scheme,100);
...
$created=new CreatedOrder(55,'ABCD12345',1000,'BGN',1,['first_name'=>'Ivan','last_name'=>'Ivanov','phone'=>'+359888123','email'=>'ivan@example.com'],['invoice'=>['address1'=>'Sofia 1'],'delivery'=>['address1'=>'Sofia 2']],[['id_product'=>42,'id_product_attribute'=>3,'name'=>'Product_Name','quantity'=>2,'total'=>1000]]);
...
assertOrder($orders->amounts===[1000.0],'validateOrder amount is not the tax-inclusive cart total');
